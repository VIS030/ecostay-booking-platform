  ...
  return (
    <div className="mx-auto max-w-[1120px] px-6 py-6 md:px-10">
      <div className="mb-4 flex flex-wrap items-start justify-between gap-4">
        <div>
          <h1 className="text-2xl font-semibold text-[#222222] md:text-[26px]">{property.title}</h1>
          <div className="mt-2 flex flex-wrap items-center gap-2 text-sm">
            <span className="font-semibold">★ {property.rating}</span>
            <span className="text-[#717171]">·</span>
            <button className="font-semibold underline">{property.reviewCount} reviews</button>
            <span className="text-[#717171]">·</span>
            <span className="font-semibold underline">{locationLabel}</span>
          </div>
        </div>
        <div className="flex gap-3">
          <button className="flex items-center gap-2 text-sm font-semibold underline">Share</button>
          <button className="flex items-center gap-2 text-sm font-semibold underline">Save</button>
        </div>
      </div>

      <div className="mb-8 grid h-[300px] gap-2 overflow-hidden rounded-xl sm:grid-cols-4 sm:grid-rows-2 md:h-[480px]">
        <button onClick={() => { setActiveImage(0); setGalleryOpen(true); }} className="relative sm:col-span-2 sm:row-span-2">
          <img src={images[0]} alt={property.title} className="h-full w-full object-cover hover:brightness-95" />
        </button>
        {images.slice(1, 5).map((img, i) => (
          <button key={i} onClick={() => { setActiveImage(i + 1); setGalleryOpen(true); }} className="relative hidden sm:block">
      ...
      <div className="grid gap-12 lg:grid-cols-[1fr_360px]">
        <div>
          <div className="border-b border-[#dddddd] pb-8">
            ...
            {property.host.superhost && (
              <div className="mt-6 border-t border-[#dddddd] pt-6">
                <p className="font-semibold text-[#222222]">{property.host.name} is a Superhost</p>
                <p className="mt-1 text-[#717171]">Superhosts are experienced, highly rated hosts.</p>
              </div>
            )}
          </div>

          <div className="border-b border-[#dddddd] py-8">
            <p className="leading-7 text-[#222222]">{property.description}</p>
          </div>
          ...
          <div className="border-b border-[#dddddd] py-8">
            ...
            <ul className="grid gap-4 sm:grid-cols-2">
              {property.amenities.map((amenity) => (
                <li key={amenity} className="flex items-center gap-3 text-[#222222]">
          ...
          <div className="border-b border-[#dddddd] py-8">
            <h2 className="mb-4 text-[22px] font-semibold">Eco credentials</h2>
            <ul className="space-y-2">
              {property.ecoFeatures.map((f) => (
                <li key={f} className="text-[#717171]">{f}</li>
              ))}
            </ul>
          </div>
