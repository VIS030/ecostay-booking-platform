...
function StarIcon() {
  return (
    <svg viewBox="0 0 32 32" className="h-3 w-3 fill-[#222222] dark:fill-slate-200">
...
export default function PropertyCard({ property, variant = 'airbnb', className = '' }) {
  ...
  if (variant === 'agoda') {
    return (
      <article className={`flex flex-col overflow-hidden rounded-lg border border-[#dddddd] dark:border-slate-800 bg-white dark:bg-slate-900 transition-shadow hover:shadow-lg sm:flex-row ${className}`}>
        ...
        <div className="flex flex-1 flex-col justify-between p-4 sm:flex-row sm:gap-4">
          <div className="flex-1">
            <Link to={`/properties/${property.slug || property.id}`}>
              <h3 className="text-lg font-bold text-[#222222] dark:text-white hover:text-[#2068a2] dark:hover:text-blue-400 hover:underline">{property.title}</h3>
            </Link>
            <p className="mt-1 text-sm text-[#717171] dark:text-slate-400">{locationLabel}</p>
            <p className="mt-1 text-xs text-[#717171] dark:text-slate-400">{property.propertyType}</p>
            <div className="mt-3 flex flex-wrap gap-2">
              {property.amenities.slice(0, 4).map((a) => (
                <span key={a} className="rounded bg-[#f0f0f0] dark:bg-slate-800 px-2 py-0.5 text-xs text-[#717171] dark:text-slate-300">{a}</span>
              ))}
            </div>
            <div className="mt-3 flex items-center gap-2">
              <span className="rounded bg-[#2068a2] px-2 py-1 text-sm font-bold text-white">{agodaScore}</span>
              <span className="text-sm font-semibold text-[#222222] dark:text-slate-200">Excellent</span>
              <span className="text-xs text-[#717171] dark:text-slate-400">({property.reviewCount} reviews)</span>
            </div>
          </div>
          <div className="mt-4 flex shrink-0 flex-col items-end justify-between border-t border-[#eeeeee] dark:border-slate-800 pt-4 sm:mt-0 sm:border-0 sm:pt-0 sm:text-right">
            <div>
              <p className="text-xs text-[#717171] dark:text-slate-400">From</p>
              <p className="text-2xl font-bold text-[#ff6600]">${property.price}</p>
              <p className="text-xs text-[#717171] dark:text-slate-400">per night</p>
            </div>
            <Link
    ...
    <article className={`group cursor-pointer ${className}`}>
      <Link to={`/properties/${property.slug || property.id}`} className="block">
        <div className="relative mb-3 aspect-square overflow-hidden rounded-xl">
          ...
          {property.featured && (
            <span className="absolute left-3 top-3 rounded bg-white dark:bg-slate-800 px-2 py-1 text-xs font-semibold text-[#222222] dark:text-slate-100 shadow">
              Guest favorite
            </span>
          )}
          <button
        ...
        <div className="space-y-0.5">
          <div className="flex items-start justify-between gap-2">
            <h3 className="truncate font-semibold text-[#222222] dark:text-slate-100">{property.location.city}, {property.location.country}</h3>
            <span className="flex shrink-0 items-center gap-0.5 text-sm text-[#222222] dark:text-slate-200">
              <StarIcon />
              <span>{property.rating}</span>
            </span>
          </div>
          <p className="truncate text-sm text-[#717171] dark:text-slate-400">{property.title}</p>
          <p className="truncate text-sm text-[#717171] dark:text-slate-400">{property.propertyType}</p>
          <p className="mt-0.5 text-sm">
            <span className="font-semibold text-[#222222] dark:text-slate-100">${property.price}</span>
